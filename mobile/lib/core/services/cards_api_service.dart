...
import 'package:qard/shared/models/card.dart';
...
class CardsApiService {
  ...
  Future<ApiResponse<List<Card>>> getBusinessCards() async {
    try {
      ...
      if (response.data is List) {
        ...
        final cards =
            cardsData.map((cardJson) => Card.fromJson(cardJson)).toList();
        return ApiResponse<List<Card>>(data: cards);
      }
  ...
  Future<ApiResponse<Card>> createBusinessCard(
  ...
  Future<ApiResponse<Card>> updateBusinessCard(
    String id,
